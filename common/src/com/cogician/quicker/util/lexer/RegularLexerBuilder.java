...
/**
 * <p>
 * Regular builder of {@linkplain QuickLexer}. This builder will build a type of
 * lexical analyzer which use regular expression.
 * </p>
 *
 * @author Fred Suvn
...
public class RegularLexerBuilder implements Buildable<QuickLexer> {
	...
	/*
	 * <p> For this lexer, each token is a {@linkplain RegularLexToken}. Parsers
	 * built by this builder have two ways to parse: one is using a whole
	 * pattern combines all patterns of given token in adding order, each in a
	 * group with a group name, and when it starts to tokenize, it will try to
	 * match by the whole pattern and find which token matched by group name;
	 * the other is using each pattern of tokens. The prefix of group name
	 * should be set to a unique value to avoid existing same group names in
	 * given token's pattern. Default the prefix is {@linkplain #GROUP_PREFIX}.
	 * If set group prefix to null, or use {@linkplain #buildEach()}, it will
	 * build lexer which use each token to match. </p>
	 */
	...
	/**
	 * <p>
	 * Adds given tokens in its order and returns this instance. Each token of
	 * Given tokens should non-contained in the added tokens, and its pattern
	...
	/**
	 * <p>
	 * Inserts given tokens at specified index in index order and returns this
	 * instance.
	 * </p>
	 * 
	 * @param index
	...
	public RegularLexerBuilder insertTokens(int index, RegularLexToken... tokens)
		...
		QuiCollections.addAll(this.tokens, tokens);
		return this;
	}
	 ...
	 * <p>
	 * Inserts given tokens at specified index in index order and returns this
	 * instance. Shifts tokens currently at specified index and subsequence
	 * after it to right. Each token of Given tokens should non-contained in the
	 * added tokens, and its pattern should be existing, or it will not be
	 * added.
	 * </p>
	 * 
	 * @param index
	...
	/**
	 * <p>
	 * Removes first occurrence of given token. If there is no given token in
	 * this builder, do nothing.
	 * </p>
	 * 
	 * @param token
	 *            given token
	...
	/**
	 * <p>
	 * Builds a {@linkplain QuickLexer} according to current build status, with
	 * parsing by way using each token to match.
	 * </p>
	 * 
	 * @return a {@linkplain QuickLexer} according to current build status
	 * @since 0.0.0
	...
	private static class EntirePatternRegularLexer extends AbstractRegularLexer {
		...
		private Pattern createPattern() {
			int[] counter = { 0 };
			String entireRegex = ToStringQuicker.join(Quicker.iterator(regularTokens.iterator(), token -> {
