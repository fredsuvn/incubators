...
public class QuickerProperties {
    ...
    private static final QConfiguration config;
     ...
     * @since 0.0.0
     */
    public static QConfiguration getProperties() {
    ...
    public static final QLogger LOG = sysLogger();

    private static QLogger sysLogger() {
        String logOut = QuickerProperties.get("log.out").asString();
        String[] outs = logOut.split(",");
        List<QLogger> list = new LinkedList<>();
        QLogger.Builder builder = new QLogger.Builder().setLevel(config.getInt("log.level"))
        ...
        return QLogger.multiLogger(list);
    }
}
